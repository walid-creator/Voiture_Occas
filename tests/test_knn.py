...
def biais(Y_True,Y_Pred):
    n = len(Y_Pred)
    somme = 0
    for i in range(n):
        somme += (Y_Pred[i] - Y_True[i]) / Y_True[i]
    return somme / n

def get_sd_error(Y_True,Y_Pred):
    n = len(Y_Pred)
    somme = 0
    m_bar = biais(Y_True, Y_Pred)
    for i in range(n):
        somme += ((Y_Pred[i] - Y_True[i]) / Y_True[i] - m_bar)**2
    return(np.sqrt(somme/n))



ecart_type=get_sd_error(np.exp(y_test), np.exp(pred))
coef_var=get_sd_error(np.exp(y_test), np.exp(pred))/biais(np.exp(y_test), np.exp(pred))
...
plt.scatter(x,y0,color="yellow",label=" R2_ajusté")
...
#densité
sns.distplot(np.exp(y_test), hist=False,kde=True, label='vraies')
sns.distplot(np.exp(pred), hist=False,kde=True, label='estimations')
axes = plt.gca()
axes.set_xlabel('axe des x')
axes.set_xlim(0, 40000)
plt.title("vraies val vs estimations")
plt.legend()
plt.show()
